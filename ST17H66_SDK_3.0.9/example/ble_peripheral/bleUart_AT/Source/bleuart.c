...
//	LOG("MAC: %x %x %x %x %x %x",AT_mac_address[0],AT_mac_address[1],AT_mac_address[2],AT_mac_address[3],AT_mac_address[4],AT_mac_address[5]) ;
	...
	osal_snv_read(MY_MODIFY_ID,MY_MODIFY_ID_LEN,&Modify_BLEDevice_Data);
	if(Modify_BLEDevice_Data==1)
	{
		osal_snv_read(MY_DEVICE_NAME_ID,MY_DEVICE_NAME_ID_LEN,&(scanRspData[2]));			// 20 Bytes Device Name
		osal_snv_read(MY_MAC_ADDRESS_ID,MY_MAC_ADDRESS_ID_LEN,AT_mac_address);				// 6 Bytes MAC address
		osal_snv_read(MY_UART_BAUDRATE_ID,MY_UART_BAUDRATE_ID_LEN,uart_baudrate);				// 4 Bytes uart_baudrate
		osal_snv_read(MY_ADV_INTERVAL_ID,MY_ADV_INTERVAL_ID_LEN,&advint);						// 1 Byte Advert interval
		osal_snv_read(MY_BLE_UART_AUTO_ID,MY_BLE_UART_AUTO_ID_LEN,&AT_bleuart_auto);				// 1 Byte BLE_UART AUTO
		osal_snv_read(MY_TX_POWER_ID,MY_TX_POWER_ID_LEN,&AT_bleuart_txpower);			// 1 Byte TX power
		osal_snv_read(MY_RESERVED_DATA_ID,MY_RESERVED_DATA_ID_LEN,&(advertdata[AT_cnt_advdata]));			// 8 Bytes reserved data
	...
	else if(Modify_BLEDevice_Data==3)						//恢复出厂设置
	{
		...
		osal_snv_write(MY_DEVICE_NAME_ID,MY_DEVICE_NAME_ID_LEN,&(scanRspData[2]));			// 20 Bytes Device Name
		osal_snv_write(MY_MAC_ADDRESS_ID,MY_MAC_ADDRESS_ID_LEN,AT_mac_address);				// 6 Bytes MAC address
	...
	else													//初始化设置
	{
		...
		osal_snv_write(MY_RESTORE_DEVICE_NAME_ID,MY_DEVICE_NAME_ID_LEN,&(scanRspData[2]));			// 20 Bytes Device Name
		osal_snv_write(MY_RESTORE_MAC_ADDRESS_ID,MY_MAC_ADDRESS_ID_LEN,AT_mac_address);				// 6 Bytes MAC address
		osal_snv_write(MY_RESTORE_UART_BAUDRATE_ID,MY_UART_BAUDRATE_ID_LEN,uart_baudrate);				// 4 Bytes uart_baudrate
		osal_snv_write(MY_RESTORE_ADV_INTERVAL_ID,MY_ADV_INTERVAL_ID_LEN,&advint);						// 1 Byte Advert interval
		osal_snv_write(MY_RESTORE_BLE_UART_AUTO_ID,MY_BLE_UART_AUTO_ID_LEN,&AT_bleuart_auto);			// 1 Byte BLE_UART AUTO
		osal_snv_write(MY_RESTORE_TX_POWER_ID,MY_TX_POWER_ID_LEN,&AT_bleuart_txpower);			// 1 Byte TX power
		osal_snv_write(MY_RESTORE_RESERVED_DATA_ID,MY_RESERVED_DATA_ID_LEN,&(advertdata[AT_cnt_advdata]));			// 8 Bytes reserved data
...
//  DevInfo_AddService();                           // Device Information Service
  ...
  ota_app_AddService();			//开启OTA服务
  //OTA升级密钥
  uint8 ota_keylen=10;
  uint8 ota_key[10]={'0','0','0','0','0','0','0','0','0','2'};
  ota_app_AddService_UseKey(ota_keylen,ota_key);
...
//	req.clientRxMTU = 247;
//	ATT_SetMTUSizeMax(req.clientRxMTU);
//	GATT_ExchangeMTU(0 , &req , bleuart_TaskID);
	extern gattCharCfg_t bleuart_TxCCCD;
	GATTServApp_WriteCharCfg( 0,&bleuart_TxCCCD, 0x0001 );
	BUP_connect_handler();
...
static void bleuart_StateNotificationCB( gaprole_States_t newState )
{
    switch ( newState )
    {
		...
		#endif
		  osal_start_timerEx( bleuart_TaskID, SBP_NOTIFY_EVT, 40 );
